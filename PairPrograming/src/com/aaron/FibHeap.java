...
public class FibHeap {
  ...
  private void decrease(FibNode node, int key) {
    ...
    if (key > node.key) {
      System.out.printf("decrease failed: the new key(%d) is " 
          + "no smaller than current key(%d)\n", key, node.key);
      return;
    }
    FibNode parent = node.parent;
    node.key = key;
  ...
  private void increase(FibNode node, int key) {
    ...
    if (key <= node.key) {
      System.out.printf("increase failed: the new key(%d) is no " 
          + "greater than current key(%d)\n", key, node.key);
      return;
    }
