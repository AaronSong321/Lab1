...
public class ListDG {
  ...
  public ListDG(File file) throws FileNotFoundException {
    ...
    try {
      ...
      /*
       * for (int i = 0; i < mVexs.length; i++){ ENode tempEdge =
       * mVexs[tokens[i]].firstEdge; System.out.print(mVexs[tokens[i]].data); while
       * (tempEdge != null){ System.out.print(" " + tempEdge.weight + " " +
       * mVexs[tempEdge.ivex].data); tempEdge = tempEdge.nextEdge; }
       * System.out.println(); }
       */
    } catch (IOException e) {
      e.printStackTrace();
  ...
  private void linkLast(int startv, ENode node) {
    if (mVexs[startv].firstEdge == null) {
      mVexs[startv].firstEdge = node;
    } else {
      ENode end = mVexs[startv].firstEdge;
      while (end.nextEdge != null) {
        end = end.nextEdge;
      }
      end.nextEdge = node;
    }
  ...
  /*
   * public void dijkstra(int vs, int[] prev, int[] dist) { FibHeap H = new
   * FibHeap(); boolean[] flag = new boolean[mVexs.length]; for (int i = 0; i <
   * mVexs.length; i++) { flag[i] = false; prev[i] = 0; dist[i] = INF; } dist[vs]
   * = 0; H.insert(vs, 0); while(H.minimum_key() != -1) { int u =
   * H.minimum_name(); H.removeMin(); if(flag[u]) continue; flag[u] = true; ENode
   * p = mVexs[u].firstEdge;
   * 
   * while(p != null){ int v = p.ivex;
   * if(flag[v]==false&&dist[v]>dist[u]+p.weight){ dist[v] = dist[u]+p.weight;
   * prev[v] = u; H.insert(v, dist[v]); } p = p.nextEdge; } } }
   */
  public int length() {
    return mVexs.length;
  }
  ...
  public void toDot() {
    ...
    try {
      ...
      for (int i = 0; i < mVexs.length; i++) {
        dotBufferedWriter.write(mVexs[i].data 
            + " [shape = ellipse, width = 1.5, height = 1.0, color = black];\n");
      }
      for (int i = 0; i < mVexs.length; i++) {
        ENode temp = mVexs[i].firstEdge;
        while (temp != null) {
          dotBufferedWriter.write(
              mVexs[i].data + " -> " + mVexs[temp.ivex].data 
              + "[color = black, label = \"" + temp.weight + "\"];\n");
          temp = temp.nextEdge;
        }
      }
      dotBufferedWriter.write("}");
