...
public class MainPairPrograming extends JFrame implements ActionListener {
  ...
  JMenu settingsMenu;
  JMenuItem importJMI;
  JMenuItem generateJMI;
  JMenuItem saveJMI;
  JMenuItem closeJMI;
  JMenuItem showJMI;
  JMenuItem bridgeWordsJMI;
  JMenuItem newTextJMI;
  JMenuItem shortestPathJMI;
  JMenuItem randomlyTravelJMI;
  JMenuItem interfaceJMI;
  JMenuItem testJMI;
  JLabel label;
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    final int temp = operation.length() - 2;
  ...
  private void generate() {
    try {
      ...
      if (fileToOpen.getName() == null) {
        ...
        label.setText("File " + fileToOpen.getName() + " is not found.");
      }
    }
  }
  ...
  private void save() {
    ...
    Command.execute(thisDirectory.getAbsolutePath() 
        + "\\dot " + dotFile.getAbsolutePath() + " -T gif -o Graph.gif");
  }
  ...
  private void showGraph() {
    ...
    File thisDirectory = new File("");
    Command.execute(
        thisDirectory.getAbsolutePath() + "\\dot " 
        + theDirectory.getAbsolutePath() + " -T gif -o GraphRunning.gif");
  ...
  private void gridset(GridBagConstraints c, int gx, int gy, 
      int gw, int gh, int ix, int iy, double wx, double wy) {
    c.gridx = gx;
    c.gridy = gy;
    c.gridwidth = gw;
    c.gridheight = gh;
  ...
  private void formulateNewText() {
    ...
    formulateNewText.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        outputNewText.setText("The result is: " 
            + GraphFunctions.generateNewText(graph, inputString));
      }
    });
  ...
  JLabel pathInputWord1;
  JLabel pathInputWord2;
  JLabel pathOutput;
  JTextField pathWord1;
  JTextField pathWord2;
  JButton pathCalculate;
  JButton pathAll;// pathNext,
  int[] theway;
  int wayIndex;
    ...
    pathCalculate.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        if (!graph.hashTable.containsKey(word1) && !graph.hashTable.containsKey(word2)) {
          ...
          pathOutput.setText(
              "The shortest path between " + word1 + " and " 
                + word2 + " is " + theway[theway.length - 1] + ".");
          wayIndex = 0;
          componentsToDraw.vertexes[theway[wayIndex++]].setColor("red");
          File thisDirectory = new File("");
          componentsToDraw.toDot("DotFileRunning.gv");
          Command.execute(thisDirectory.getAbsolutePath() 
              + "\\dot " + thisDirectory.getAbsolutePath() + " -T gif -o "
              + thisDirectory.getAbsolutePath() + "\\GraphRunning.gif");
        }
      }
    });
    /*
     * pathNext.addActionListener(new ActionListener(){ public void
     * actionPerformed(ActionEvent e){ for (int i = 0; i <
     * componentsToDraw.edges.length; i++) if (componentsToDraw.edges[i].src ==
     * componentsToDraw.vertexes[theway[wayIndex-1]].name &&
     * componentsToDraw.edges[i].dst ==
     * componentsToDraw.vertexes[theway[wayIndex]].name) {
     * componentsToDraw.edges[i].setColor("red"); break; } wayIndex++; File
     * thisDirectory = new File(""); componentsToDraw.toDot("DotFileRunning.gv");
     * Command.execute(DotPath+" "+thisDirectory.getAbsolutePath()
     * +" -T gif -o GraphRunning.gif"); } });
     */
    pathAll.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        while (wayIndex != theway.length - 1) {
          for (int i = 0; i < componentsToDraw.edges.length; i++) {
            if (componentsToDraw.edges[i].src
                .equals(componentsToDraw.vertexes[theway[wayIndex - 1]].name)
                && componentsToDraw.edges[i].dst
                .equals(componentsToDraw.vertexes[theway[wayIndex]].name)) {
        ...
        if (oldRunningGraph.exists()) {
          oldRunningGraph.delete();
        }
        File thisDirectory = new File("");
        componentsToDraw.toDot("DotFileRunning.gv");
        Command.execute(thisDirectory.getAbsolutePath() 
  ...
  JButton travelStart;
  JButton travelStop;// travelContinue,
  JLabel travel;
  ...
  private void randomTravel() {
    ...
    travelStart.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        placeIndex = 0;
        travel.setText("The start word of this travel is " 
            + componentsToDraw.vertexes[places[placeIndex]].name);
    ...
    /*
     * travelContinue.addActionListener(new ActionListener(){ public void
     * actionPerformed(ActionEvent e){ if (placeIndex == places.length){
     * travel.setText("The travel has finished."); travelStop.setEnabled(false);
     * travelContinue.setEnabled(false); } else {
     * travel.setText("Travel to "+componentsToDraw.vertexes[places[placeIndex]].
     * name); componentsToDraw.vertexes[places[placeIndex]].setColor("red"); for
     * (int i = 0; i < componentsToDraw.edges.length; i++) if
     * (componentsToDraw.edges[i].src ==
     * componentsToDraw.vertexes[places[placeIndex-1]].name &&
     * componentsToDraw.edges[i].dst ==
     * componentsToDraw.vertexes[places[placeIndex]].name) {
     * componentsToDraw.edges[i].setColor("red"); break; } placeIndex++; File
     * thisDirectory = new File(""); componentsToDraw.toDot("DotFileRunning.gv");
     * Command.execute(DotPath+" "+thisDirectory.getAbsolutePath()
     * +" -T gif -o GraphRunning.gif"); } } });
     */
    travelStop.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        for (placeIndex = 1; placeIndex < places.length; placeIndex++) {
          for (int i = 0; i < componentsToDraw.edges.length; i++) {
            if (componentsToDraw.edges[i].src
                .equals(componentsToDraw.vertexes[places[placeIndex - 1]].name)
                && componentsToDraw.edges[i].dst
                .equals(componentsToDraw.vertexes[places[placeIndex]].name)) {
              componentsToDraw.edges[i].setColor("red");
              System.out.println("The edge from " + componentsToDraw.edges[i].src + " to "
                  + componentsToDraw.edges[i].dst + " is found.");
              break;
            }
          }
        }
